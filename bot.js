...
if (!chatId) {
    console.error('Ошибка: CHAT_ID не найден в файле .env');
    process.exit(1);
}
...
function getRandomMorningMessage() {
    ...
    return `ТЕСТ: ${morningMessages[randomIndex]}`;
}
...
async function sendMorningMessage() {
    try {
        ...
        console.log(`Утреннее сообщение отправлено: ${new Date().toLocaleString('ru-RU')}`);
    } catch (error) {
        console.error('Ошибка при отправке утреннего сообщения:', error.message);

        // Если ошибка связана с неправильным chat_id, выводим подсказку
        if (error.message.includes('chat not found')) {
            console.log('Возможно, неправильный CHAT_ID. Убедитесь, что бот добавлен в группу и имеет права на отправку сообщений.');
        }
    }
}

// Настройка cron задания для отправки сообщения каждое утро в 8:00 '0 0 8 * * *'
// Формат: секунды минуты часы день месяц день_недели
// - Каждое утро в 8:00: '0 0 8 * * *'
// - Каждые 15 сек: '*/15 * * * * *'
//
cron.schedule('*/15 * * * * *', () => {
    console.log('Время отправить утреннее приветствие!');
    sendMorningMessage();
}, {
    scheduled: true,
...
bot.onText(/\/start/, (msg) => {
    ...
    bot.sendMessage(chatId, 'Привет! Я бот для утренних приветствий. Каждое утро в 8:00 я буду отправлять доброе утро в группу!');
